...
if __name__ == "__main__":
    ...
    while counter < num_iters:
        ...
        query = {
            'query': 'sum(container_memory_usage_bytes) by (container_label_com_docker_compose_service)'}
        parse_result(query, ['container_label_com_docker_compose_service'], 'memory', is_first=is_first)
        ...
        # TODO: 1m?
        query = {
            'query': 'sum(rate(container_cpu_usage_seconds_total{}[1m])) by (container_label_com_docker_compose_service)'}
        parse_result(query, ["container_label_com_docker_compose_service"],
                     'CPU_percentage_pod', is_first=is_first)

        # Network transmited by pod, namespace
        query = {
            'query': 'sum(container_network_transmit_bytes_total) by (container_label_com_docker_compose_service'}
        parse_result(query, ['container_label_com_docker_compose_service'],
                     'network_transmit', is_first=is_first)

        # Network received by pod, namespace
        query = {
            'query': 'sum(container_network_receive_bytes_total) by (container_label_com_docker_compose_service)'}
        parse_result(query, ['container_label_com_docker_compose_service'],
        ...
        query={'query':'sum(container_fs_reads_bytes_total) by (container_label_com_docker_compose_service)'}
        parse_result(query, ['container_label_com_docker_compose_service'], 'fs_read', is_first=is_first)

        # FS write bytes by pod, namespace
        query={'query':'sum(container_fs_writes_bytes_total) by (container_label_com_docker_compose_service)'}
        parse_result(query, ['container_label_com_docker_compose_service'], 'fs_write', is_first=is_first)
