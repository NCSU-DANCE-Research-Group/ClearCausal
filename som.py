...
class SOM():
    ...
    def find_bmu(self, x):
        """
        Find the index and the distance of the best matching unit for the input vector x.
        """
        # Stack x to have one row per weight
        x_stack = np.stack([x]*(self.m*self.n), axis=0)
        # Calculate distance between x and each weight
        distances = np.linalg.norm(x_stack - self.weights, axis=1)
        # Find index of best matching unit
        bmu_index = np.argmin(distances)
        # Find the minimum distance
        bmu_distance = distances[bmu_index]
        return bmu_index, bmu_distance
